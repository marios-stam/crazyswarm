...
void logWarn(const std::string &msg) { ROS_WARN("%s", msg.c_str()); }
...
public:
    ...
    virtual void info(const std::string &msg) { ROS_INFO("%s", msg.c_str()); }

    virtual void warning(const std::string &msg) { ROS_WARN("%s", msg.c_str()); }

    virtual void error(const std::string &msg) { ROS_ERROR("%s", msg.c_str()); }
};
...
{
public:
    CrazyflieROS(const std::string &link_uri, const std::string &tf_prefix, const std::string &frame, const std::string &worldFrame, int id,
                 const std::string &type, const std::vector<crazyswarm::LogBlock> &log_blocks, ros::CallbackQueue &queue)
        : m_tf_prefix(tf_prefix), m_cf(link_uri, rosLogger, std::bind(&CrazyflieROS::onConsole, this, std::placeholders::_1)), m_frame(frame),
          m_worldFrame(worldFrame), m_id(id), m_type(type), m_serviceUpdateParams(), m_serviceUploadTrajectory(), m_serviceStartTrajectory(),
          m_serviceTakeoff(), m_serviceLand(), m_serviceGoTo(), m_serviceSetGroupMask(), m_serviceNotifySetpointsStop(), m_logBlocks(log_blocks),
...
public:
    template <class T, class U> void updateParam(uint16_t id, const std::string &ros_param)
    {
        U value;
    ...
    bool updateParams(crazyswarm::UpdateParams::Request &req, crazyswarm::UpdateParams::Response &res)
    {
    ...
    bool uploadTrajectory(crazyswarm::UploadTrajectory::Request &req, crazyswarm::UploadTrajectory::Response &res)
    {
        ...
        {
            if (req.pieces[i].poly_x.size() != 8 || req.pieces[i].poly_y.size() != 8 || req.pieces[i].poly_z.size() != 8 ||
    ...
    bool startTrajectory(crazyswarm::StartTrajectory::Request &req, crazyswarm::StartTrajectory::Response &res)
    {
    ...
    bool notifySetpointsStop(crazyswarm::NotifySetpointsStop::Request &req, crazyswarm::NotifySetpointsStop::Response &res)
    {
    ...
    bool takeoff(crazyswarm::Takeoff::Request &req, crazyswarm::Takeoff::Response &res)
    {
    ...
    bool land(crazyswarm::Land::Request &req, crazyswarm::Land::Response &res)
    {
    ...
    bool goTo(crazyswarm::GoTo::Request &req, crazyswarm::GoTo::Response &res)
    {
    ...
    bool setGroupMask(crazyswarm::SetGroupMask::Request &req, crazyswarm::SetGroupMask::Response &res)
    {
    ...
    void cmdPositionSetpoint(const crazyswarm::Position::ConstPtr &msg)
    {
        // if(!m_isEmergency) {
        float x = msg->x;
        float y = msg->y;
    ...
    void cmdFullStateSetpoint(const crazyswarm::FullState::ConstPtr &msg)
    {
        ...
        m_cf.sendFullStateSetpoint(x, y, z, vx, vy, vz, ax, ay, az, qx, qy, qz, qw, rollRate, pitchRate, yawRate);
        // m_sentSetpoint = true;
    ...
    void cmdVelocityWorldSetpoint(const crazyswarm::VelocityWorld::ConstPtr &msg)
    {
        ...
        m_cf.sendVelocityWorldSetpoint(x, y, z, yawRate);
        // m_sentSetpoint = true;
        ...
        if (m_enableLogging)
        {
            ...
            for (auto &logBlock : m_logBlocks)
            {
                std::function<void(uint32_t, std::vector<double> *, void *userData)> cb =
                    std::bind(&CrazyflieROS::onLogCustom, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);

                m_logBlocksGeneric[i].reset(new LogBlockGeneric(&m_cf, logBlock.variables, (void *)&m_pubLogDataGeneric[i], cb));
            ...
            if (m_enableLoggingPose)
            {
                std::function<void(uint32_t, logPose *)> cb =
                    std::bind(&CrazyflieROS::onPoseData, this, std::placeholders::_1, std::placeholders::_2);

                m_logBlockPose.reset(new LogBlock<logPose>(
                    &m_cf, {{"stateEstimate", "x"}, {"stateEstimate", "y"}, {"stateEstimate", "z"}, {"stateEstimateZ", "quat"}}, cb));
    ...
    const Crazyflie::ParamTocEntry *getParamTocEntry(const std::string &group, const std::string &name) const
    {
...
public:
    ...
    CrazyflieGroup(const std::vector<libobjecttracker::DynamicsConfiguration> &dynamicsConfigurations,
                   const std::vector<libobjecttracker::MarkerConfiguration> &markerConfigurations, pcl::PointCloud<pcl::PointXYZ>::Ptr pMarkers,
                   std::map<std::string, libmotioncapture::RigidBody> *pMocapRigidBodies, int radio, int channel, bool useMotionCaptureObjectTracking,
                   const std::vector<crazyswarm::LogBlock> &logBlocks, std::string interactiveObject, bool writeCSVs, bool sendPositionOnly)
        : m_cfs(), m_tracker(nullptr), m_radio(radio), m_pMarkers(pMarkers), m_pMocapRigidBodies(pMocapRigidBodies), m_slowQueue(),
          m_cfbc("radio://" + std::to_string(radio) + "/" + std::to_string(channel) + "/2M/FFE7E7E7E7"), m_isEmergency(false),
          m_useMotionCaptureObjectTracking(useMotionCaptureObjectTracking), m_br(), m_interactiveObject(interactiveObject),
          m_sendPositionOnly(sendPositionOnly), m_outputCSVs(), m_phase(0), m_phaseStart()
    {
    ...
    const latency &lastLatency() const { return m_latency; }

    int radio() const { return m_radio; }

    void runInteractiveObject(std::vector<CrazyflieBroadcaster::externalPose> &states) { publishRigidBody(m_interactiveObject, 0xFF, states); }
        ...
        else
        {
            ...
            for (size_t i = 0; i < m_cfs.size(); ++i)
            {
                if (m_tracker->objects()[i].lastTransformationValid())
                {
                    ...
                    states.back().qw = q.w();
                    ROS_INFO("=============================START=============================");
                    ROS_INFO("%d: %f %f %f", m_cfs[i]->id(), translation.x(), translation.y(), translation.z());
                    ROS_INFO("=============================END=============================");
                    ...
                    if (m_outputCSVs.size() > 0)
                    {
                        ...
                        *m_outputCSVs[i] << t << "," << states.back().x << "," << states.back().y << "," << states.back().z << "," << rpy(0) << ","
                                         << rpy(1) << "," << rpy(2) << "\n";
                    }
                }
                else
                {
                    ...
                    ROS_WARN("No updated pose for CF %s for %f s.", m_cfs[i]->frame().c_str(), elapsedSeconds.count());
                }
            }
        }
    ...
    void startTrajectory(uint8_t trajectoryId, float timescale, bool reversed, uint8_t groupMask)
    {
        // for (size_t i = 0; i < 10; ++i) {
    ...
    template <class T, class U> void updateParam(const char *group, const char *name, const std::string &ros_param)
    {
        U value;
        ros::param::get(ros_param, value);
    ...
    void updateParams(const std::vector<std::string> &params)
    {
...
private:
    bool publishRigidBody(const std::string &name, uint8_t id, std::vector<CrazyflieBroadcaster::externalPose> &states)
    {
        ...
        if (iter != m_pMocapRigidBodies->end())
        {
            ...
            ROS_INFO("%s: %f, %f, %f", name.c_str(), states.back().x, states.back().y, states.back().z);
            ...
            transform.setOrigin(tf::Vector3(states.back().x, states.back().y, states.back().z));
            tf::Quaternion q(states.back().qx, states.back().qy, states.back().qz, states.back().qw);
    ...
    void readObjects(std::vector<libobjecttracker::Object> &objects, int channel, const std::vector<crazyswarm::LogBlock> &logBlocks)
    {
        // read CF config
        struct CFConfig
        {
                ...
                for (int32_t j = 0; j < pos.size(); ++j)
                {
                    ...
                    default:
                        ...
                        message << "crazyflies.yaml error:"
                                   " entry "
                                << j << " of initialPosition for cf" << id << " should be type int or double.";
    ...
    void addCrazyflie(const std::string &uri, const std::string &tf_prefix, const std::string &frame, const std::string &worldFrame, int id,
                      const std::string &type, const std::vector<crazyswarm::LogBlock> &logBlocks)
    {
        ROS_INFO("Adding CF: %s (%s, %s)...", tf_prefix.c_str(), uri.c_str(), frame.c_str());
        auto start = std::chrono::high_resolution_clock::now();
        CrazyflieROS *cf = new CrazyflieROS(uri, tf_prefix, frame, worldFrame, id, type, logBlocks, m_slowQueue);
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed = end - start;
    ...
    void updateParams(CrazyflieROS *cf)
    {
        ros::NodeHandle n("~");
        ros::NodeHandle nGlobal;
        // update parameters
...
public:
    CrazyflieServer()
        : m_isEmergency(false), m_serviceEmergency(), m_serviceStartTrajectory(), m_serviceTakeoff(), m_serviceLand(), m_serviceGoTo(),
          m_lastInteractiveObjectPosition(-10, -10, 1), m_broadcastingNumRepeats(15), m_broadcastingDelayBetweenRepeatsMs(1)
    {
        ros::NodeHandle nh;
    ...
    {
        m_lastInteractiveObjectPosition = Eigen::Vector3f(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
    }
    ...
    void runFast()
    {
        ...
        {
            ROS_INFO("libmotioncapture connecting to %s at hostname '%s' - "
                     "might block indefinitely if unreachable!",
                     motionCaptureType.c_str(), hostname.c_str());
            ...
            for (int channel : channels)
            {
                auto handle = std::async(
                    ...
                    [&](int channel, int radio)
                    {
                        ...
                        return new CrazyflieGroup(dynamicsConfigurations, markerConfigurations,
                                                  // &client,
                                                  markers, &mocapRigidBodies, radio, channel, useMotionCaptureObjectTracking, logBlocks,
                                                  interactiveObject, writeCSVs, sendPositionOnly);
                    },
                    channel, r);
                ...
                if (useMotionCaptureObjectTracking || !interactiveObject.empty())
                {
                    ...
                    if (interactiveObject == "virtual")
                    {
                        ...
                        mocapRigidBodies.emplace(interactiveObject,
                                                 libmotioncapture::RigidBody(interactiveObject, m_lastInteractiveObjectPosition, quat));
                    }
                }
...
private:
    ...
    //
    void readMarkerConfigurations(std::vector<libobjecttracker::MarkerConfiguration> &markerConfigurations)
    {
    ...
    void readDynamicsConfigurations(std::vector<libobjecttracker::DynamicsConfiguration> &dynamicsConfigurations)
    {
        ros::NodeHandle nGlobal;
        int numConfigurations;
    ...
    void readChannels(std::set<int> &channels)
    {
        // read CF config
        ros::NodeHandle nGlobal;
